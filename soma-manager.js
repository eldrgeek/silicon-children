 ...
 * See mike-wolf-com/soma-manager.js for canonical source.
 */
(function () {
  'use strict';
  ...
  function mount() {
    ...
    fab.addEventListener('click', function () { panel.classList.toggle('smgr-open'); });
    close.addEventListener('click', function () { panel.classList.remove('smgr-open'); });
    ...
    function doAsk() {
      ...
      fetch(ep.ask, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ question: q }) })
    ...
    askInput.addEventListener('keydown', function (e) { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); doAsk(); } });
    ...
    reportBtn.addEventListener('click', function () {
      ...
      fetch(ep.feedback, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ type: reportType.value, message: msg, page: window.location.href }) })
        .then(function () {
        ...
        .then(function () { reportBtn.disabled = false; reportBtn.textContent = 'Submit'; });
    });
    ...
    setTimeout(function () {
      try {
        if (window.SomaOwner && typeof window.SomaOwner.isOwner === 'function' && window.SomaOwner.isOwner()) {
          ...
          if (link) link.innerHTML = ' · <a href="/soma-owner.html" style="color:' + esc(p) + '">Owner Panel</a>';
        }
      } catch (_) {}
    }, 600);
  }
